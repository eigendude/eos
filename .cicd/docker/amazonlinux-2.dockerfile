...
COPY ./docker/.helpers-v21 /tmp/.helpers

CMD bash -c ". /tmp/.helpers && \
    $PRE_COMMANDS fold-execute ccache -s && \
    ...
    if $ENABLE_PARALLEL_TESTS; then fold-execute echo ctest -j$(getconf _NPROCESSORS_ONLN) -LE _tests --output-on-failure -T Test; fi && \
    if $ENABLE_SERIAL_TESTS; then fold-execute echo ctest -L nonparallelizable_tests --output-on-failure -T Test; fi && \
    if $ENABLE_LR_TESTS; then fold-execute echo ctest -L long_running_tests --output-on-failure -T Test; fi && \
