...
ENV PRE_COMMANDS="echo \"BUILDKITE_AGENT_TOKEN: $BUILDKITE_AGENT_TOKEN\" && buildkite-agent start && touch test && buildkite-agent artifact upload test && source /opt/rh/devtoolset-8/enable && source /opt/rh/rh-python36/enable && export PATH=/usr/lib64/ccache:\$PATH"
