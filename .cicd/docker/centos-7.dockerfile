...
ENV PRE_COMMANDS="touch test && fold-execute buildkite-agent artifact upload ./test --agent-access-token \$BUILDKITE_AGENT_ACCESS_TOKEN && source /opt/rh/devtoolset-8/enable && source /opt/rh/rh-python36/enable && export PATH=/usr/lib64/ccache:\$PATH"
